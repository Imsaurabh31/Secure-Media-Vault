...
-- Policies
create policy "owner can crud own assets" on public.asset 
  for all to authenticated 
  using (owner_id = auth.uid()) 
  with check (owner_id = auth.uid());

create policy "shared users can read" on public.asset 
...
create policy "owner manages shares" on public.asset_share 
  for all to authenticated 
  using (
    exists(
...
create policy "owner manages ticket" on public.upload_ticket 
  for all to authenticated 
  using (user_id = auth.uid()) 
  with check (user_id = auth.uid());

create policy "users can audit own downloads" on public.download_audit 
  for all to authenticated 
  using (user_id = auth.uid()) 
  with check (user_id = auth.uid());
...
create index idx_upload_ticket_user on public.upload_ticket(user_id);
create index idx_download_audit_user on public.download_audit(user_id);
